...
struct Vertex {
	...
	static VkVertexInputBindingDescription getBindingDescription() {
		...
		bindingDescription.inputRate = VK_VERTEX_INPUT_RATE_VERTEX; // for each VERTEX or for each INSTANCE (rn for VERTEX)
...
// uniform objects need to be correctly aligned
// scalars by N				-> 4 bytes
// vec2 by 2N				-> 8 bytes
// vec3/vec4 by 4N			-> 16 bytes
// inside nested structure	-> multiple of 16
// mat4 == vec4				-> 16 bytes
// to align, c++11 has alignas(x)
// ex. alignas(16) glm::mat4 model;
...
private:
	...
	void createLogicalDevice() {
		...
		deviceFeatures.sampleRateShading = VK_TRUE; // multisampling for the fragments of a texture
	...
	void createDescriptorSetLayout() {
		...
		samplerLayoutBinding.pImmutableSamplers = nullptr; // image sampling
	...
	void createGraphicsPipeline() {
		...
		depthStencilInfo.stencilTestEnable = VK_FALSE; // stencil stuff
		depthStencilInfo.front = {}; // stencil stuff
		depthStencilInfo.back = {}; // stencil stuff
		...
		multisamplingInfo.sampleShadingEnable = VK_TRUE; // multisampling of the fragments of texture
		multisamplingInfo.rasterizationSamples = msaaSamples;
		multisamplingInfo.minSampleShading = 0.2f; // closer to 1 is smoother
	...
	void createRenderPass() {
		...
		colorAttachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // layout of the pixels in memory after rendering
		// without multisampling is VK_IMAGE_LAYOUT_PRESENT_SRC_KHR because multisampled cannot be directly presented
		...
		depthAttachment.storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE; // we don't use it after rendering
		...
		colorAttachmentResolve.loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE; // what to do before rendering
		colorAttachmentResolve.storeOp = VK_ATTACHMENT_STORE_OP_STORE; // what to do after rendering
		colorAttachmentResolve.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		colorAttachmentResolve.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		colorAttachmentResolve.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // layout of the pixels in memory before rendering
		colorAttachmentResolve.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // layout of the pixels in memory after rendering
		// without multisampling is VK_IMAGE_LAYOUT_PRESENT_SRC_KHR because multisampled cannot be directly presented
	...
	void createImage(uint32_t width, uint32_t height,
		...
		imageInfo.imageType = VK_IMAGE_TYPE_2D; // 1D, 2D or 3D
		...
		imageInfo.tiling = tiling; // VK_IMAGE_TILING_LINEAR for accessing texels directly in memory
		imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		imageInfo.usage = usage;
		// VK_IMAGE_USAGE_TRANSFER_DST_BIT because the image recieves info
		// VK_IMAGE_USAGE_SAMPLED_BIT because it will be sampled in a shader
		imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		imageInfo.samples = numSamples;
		imageInfo.flags = 0; // this could use a flag for sparse images (with positions with no value)
	...
	// mipmaps are generally not created in runtime.
	// they are usually prebaked and stored in the texture file to improve loading speed
	void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, uint32_t texHeight, uint32_t mipLevels) {
		...
		for (uint32_t i = 1; i < mipLevels; ++i) {
			// transition from dst (write) to src (read)
			...
			vkCmdBlitImage(commandBuffer, // its a transfer operation, so use a transfer-valid queue
	...
	void createTextureImage() {
		...
		createImage(texWidth, texHeight, 
			...
			VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, // src because we need to create the mipmap 
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			textureImage, textureImageMemory, 
			textureMipLevels);

		transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, 
			VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, textureMipLevels);

		copyBufferToImage(stagingBuffer, textureImage, 
			static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));

		//transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, 
		//	VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, textureMipLevels);
	...
	void createTextureSampler() {
		...
		samplerInfo.magFilter = VK_FILTER_LINEAR; // more fragments than texels
		samplerInfo.minFilter = VK_FILTER_LINEAR; // more texels than fragments
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.model = glm::rotate(glm::mat4(1.0f), glm::radians(60.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	// multiple for each frame that can be rendered simultaniously
