...
private:
	...
	struct QueueFamilyIndices {
		...
		bool isCompleted() {
			return graphicsFamily.has_value() && 
				...
				transferFamily.has_value(); //&& other queues;
		}
	};
	...
	void createBuffer(VkDeviceSize size, 
		...
		bufferInfo.usage = usage; // can have more usages with | operator
		bufferInfo.sharingMode = VK_SHARING_MODE_CONCURRENT;
		bufferInfo.queueFamilyIndexCount = 2;
		QueueFamilyIndices queueFamilyIndices = findQueueFamilies(physicalDevice);
		std::vector<uint32_t> pQueueFamilies = { queueFamilyIndices.graphicsFamily.value(), queueFamilyIndices.transferFamily.value() };
		bufferInfo.pQueueFamilyIndices = pQueueFamilies.data();
	...
	void createVertexBuffer() {
		...
		createBuffer(bufferSize, 
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT, // can be source of memory transfer
		...
		createBuffer(bufferSize, 
			VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, // can be destination of memory transfer | vertex memory
